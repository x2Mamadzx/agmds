...
type FormData = {
  nom: string; entreprise: string; service: string;
  courriel: string; telephone: string; message: string;
};
...
const SLOT_ZONES: string[][] = [
  ['22%', '28%', '34%'], ['46%', '54%', '62%'], ['24%', '30%', '36%'], ['48%', '56%', '64%'],
];
...
function useRotatingBubbles() {
  const [items, setItems] = useState(() =>
    SLOT_SIDES.map((side, i) => ({ ...BUBBLE_POOL[i % BUBBLE_POOL.length], top: SLOT_ZONES[i][0], side })),
  );
...
export default function Reserver() {
  ...
  const goTo = (targetIdx: number, dir: number) => { setFieldError(null); setDirection(dir); setStepIdx(targetIdx); };
  const goNext = () => { if (stepIdx < STEP_ORDER.length - 1) goTo(stepIdx + 1, 1); };
  const goBack = () => { if (stepIdx > 0) goTo(stepIdx - 1, -1); };

  const { markConverted } = useVisitTracking();

  const submit = () => {
    createLead.mutate(
      { nom: form.nom, entreprise: form.entreprise || undefined, courriel: form.courriel, telephone: form.telephone, service: form.service, message: form.message || undefined },
      { onSuccess: () => { markConverted(); goNext(); } },
    );
  };
  ...
  const handleTextEnter = (e: React.KeyboardEvent, canAdvance: boolean, isLast: boolean, errorMessage?: string) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      e.preventDefault();
      if (!canAdvance) { if (errorMessage) setFieldError(errorMessage); return; }
      setFieldError(null);
      if (isLast) submit(); else goNext();
    }
  };

  const attemptNext = (canAdvance: boolean, errorMessage: string) => {
    if (!canAdvance) { setFieldError(errorMessage); return; }
    setFieldError(null);
    goNext();
  };
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#f2f2f2] relative overflow-hidden flex flex-col">
      <motion.div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,rgba(200,146,42,0.15)_0%,transparent_55%)]"
        animate={{ opacity: [0.6, 1, 0.6] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }} />

      {/* Side bubbles — desktop */}
      {step !== 'done' && (
        <div className="hidden lg:block absolute inset-0 pointer-events-none overflow-hidden">
          {sideBubbles.map((bubble, i) => {
            const Icon = bubble.icon;
            return (
              <div key={i} className={`absolute ${bubble.side === 'left' ? 'left-[3%]' : 'right-[3%]'}`}
                ...
                <AnimatePresence mode="wait">
                  <motion.div key={bubble.text}
                    ...
                    animate={{ opacity: 1, scale: 1, y: [0, -8, 0], x: bubble.side === 'left' ? [0, 5, 0] : [0, -5, 0] }}
                    exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.5 } }}
                    transition={{ opacity: { duration: 0.6 }, scale: { duration: 0.6 }, y: { duration: 5 + i, repeat: Infinity, ease: 'easeInOut' }, x: { duration: 5 + i, repeat: Infinity, ease: 'easeInOut' } }}>
      ...
      {step !== 'done' && (
        <div className="flex lg:hidden fixed bottom-3 left-0 right-0 justify-center pointer-events-none z-0 px-6">
          <AnimatePresence mode="wait">
            <motion.div key={sideBubbles[0].text}
              ...
              exit={{ opacity: 0, scale: 0.9, y: 6, transition: { duration: 0.4 } }} transition={{ duration: 0.5 }}>
              {(() => { const Icon = sideBubbles[0].icon; return <Icon className="w-3 h-3 text-[#C8922A] shrink-0" />; })()}
          ...
          {step !== 'done' && (
            <div className="flex items-center gap-4">
              <button onClick={goBack} disabled={stepIdx === 0}
                className="flex items-center gap-1.5 text-xs text-black/40 hover:text-black/70 disabled:opacity-0 disabled:pointer-events-none transition-colors">
                <ArrowLeft className="w-3.5 h-3.5" /> Retour
              </button>
              <Link href="/" className="text-[11px] text-black/25 hover:text-black/60 transition-opacity">Aller au site</Link>
            </div>
          )}
        </div>
        <div className="h-1 w-full bg-black/8 rounded-full overflow-hidden">
          <motion.div className="h-full bg-gradient-to-r from-[#C8922A] to-[#F5C842]"
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            {step === 'nom' && (
              <motion.div key="nom" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit">
                ...
                <input ref={inputRef as React.RefObject<HTMLInputElement>} value={form.nom}
                  onChange={(e) => setForm(p => ({ ...p, nom: e.target.value }))}
                  onKeyDown={(e) => handleTextEnter(e, nomValid, false)}
                  placeholder="Nom" className={INPUT_CLASS} />
                <div className="mt-8 flex items-center gap-4">
                  <motion.span whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="inline-flex">
                    <Button size="lg" disabled={!nomValid} onClick={goNext}
                      className="px-10 h-14 font-bold uppercase tracking-widest text-sm shadow-[0_0_30px_rgba(200,146,42,0.25)]">
            ...
            {step === 'service' && (
              <motion.div key="service" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit">
                ...
                <h2 className="text-2xl md:text-4xl font-black text-black leading-tight mb-8">
                  Quel service vous intéresse le plus, {form.nom.split(' ')[0] || ''} ?
                </h2>
            ...
            {step === 'entreprise' && (
              <motion.div key="entreprise" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit">
                ...
                <input ref={inputRef as React.RefObject<HTMLInputElement>} value={form.entreprise}
                  onChange={(e) => setForm(p => ({ ...p, entreprise: e.target.value }))}
                  onKeyDown={(e) => handleTextEnter(e, true, false)}
                  placeholder="Entreprise (optionnel)" className={INPUT_CLASS} />
                <div className="mt-8 flex items-center gap-4">
            ...
            {step === 'courriel' && (
              <motion.div key="courriel" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit">
                ...
                <input ref={inputRef as React.RefObject<HTMLInputElement>} type="email" value={form.courriel}
                  ...
                  placeholder="Courriel" className={INPUT_CLASS} />
                {fieldError && (
                  <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-sm text-red-500 mt-3">{fieldError}</motion.p>
                )}
                <div className="mt-8 flex items-center gap-4">
                  <Button size="lg" onClick={() => attemptNext(courrielValid, 'Ce courriel ne semble pas valide. Vérifiez-le et réessayez.')}
                    className="px-8 h-14 font-bold uppercase tracking-widest text-sm">
                    Suivant <ArrowRight className="w-4 h-4 ml-2" />
                  </Button>
                  <span className="text-xs text-black/35">ou appuyez sur Entrée ↵</span>
                </div>
            ...
            {step === 'telephone' && (
              <motion.div key="telephone" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit">
                ...
                <input ref={inputRef as React.RefObject<HTMLInputElement>} type="tel" value={form.telephone}
                  ...
                  placeholder="Téléphone" className={INPUT_CLASS} />
                {fieldError && (
                  <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} className="text-sm text-red-500 mt-3">{fieldError}</motion.p>
                )}
                <div className="mt-8 flex items-center gap-4">
                  <Button size="lg" onClick={() => attemptNext(telephoneValid, 'Ce numéro ne semble pas être un numéro valide au Québec.')}
                    className="px-8 h-14 font-bold uppercase tracking-widest text-sm">
                    Suivant <ArrowRight className="w-4 h-4 ml-2" />
                  </Button>
                  <span className="text-xs text-black/35">ou appuyez sur Entrée ↵</span>
            ...
            {step === 'message' && (
              <motion.div key="message" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit">
                ...
                <textarea ref={inputRef as React.RefObject<HTMLTextAreaElement>} rows={3} value={form.message}
                  onChange={(e) => setForm(p => ({ ...p, message: e.target.value }))}
                  onKeyDown={(e) => handleTextEnter(e, true, true)}
                  placeholder="Message (optionnel)" className={INPUT_CLASS + ' resize-none'} />
                <div className="mt-8 flex items-center gap-4">
                  <Button size="lg" disabled={createLead.isPending} onClick={submit}
                    className="px-8 h-14 font-bold uppercase tracking-widest text-sm">
                ...
                <p className="text-[10px] text-black/25 mt-6">
                  ...
                  <Link href="/politique-de-confidentialite" className="underline hover:text-black/45">politique de confidentialité</Link>.
                </p>
              </motion.div>
            )}
